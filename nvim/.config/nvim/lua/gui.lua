...
function tab_label(tab_number)
	...
	local icon = buffer_icon(buffer_number, vim.bo[buffer_number].buftype, vim.bo[buffer_number].filetype)

	local label = nil
	if vim.bo[buffer_number].filetype == 'netrw' then
	...
	elseif vim.bo[buffer_number].buftype == 'quickfix' then
		label = 'Quickfix List'
